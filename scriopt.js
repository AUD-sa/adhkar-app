...
const morningOverallProgress = document.getElementById('morning-overall-progress');
const eveningOverallProgress = document.getElementById('evening-overall-progress');
...
function toggleTheme() {
   ...
   themeIcon.classList.toggle('fa-adjust');
      themeIcon.classList.toggle('fa-moon');
       ...
       updateOverallProgress(); // تحديث التقدم الكلي عند تغيير الثيم
 }
const savedTheme = localStorage.getItem('theme');
 ...
 function updateOverallProgress() {
    const morningTotal = dhikrData.morning.reduce((sum, dhikr) => sum + dhikr.times, 0);
    let morningCompleted = 0;
    for (const id in dhikrCounter.morning) {
        morningCompleted += dhikrCounter.morning[id];
    }
    const morningProgress = morningCompleted / morningTotal;
    morningOverallProgress.style.width = `${(morningProgress * 100) || 0}%`;

    const eveningTotal = dhikrData.evening.reduce((sum, dhikr) => sum + dhikr.times, 0);
    let eveningCompleted = 0;
    for (const id in dhikrCounter.evening) {
        eveningCompleted += dhikrCounter.evening[id];
    }
    const eveningProgress = eveningCompleted / eveningTotal;
    eveningOverallProgress.style.width = `${(eveningProgress * 100) || 0}%`;
}
...
updateOverallProgress(); // تحديث التقدم الكلي عند تحديث تقدم الذكر الفردي
}
...
const countBtn = document.getElementById(`${type}-count-btn`);
 ...
 function goBack() {
     ...
     currentDhikrType = null;
        ...
        updateOverallProgress(); // تحديث التقدم الكلي عند العودة للصفحة الرئيسية
    }
  function toggleSidebar() {
   ...
   updateOverallProgress(); // تهيئة التقدم الكلي عند تحميل الصفحة