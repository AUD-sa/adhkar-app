...
// تعريف صفحة عداد الذكر
const counterList = document.getElementById('counter-list');

function openCounterMenu() {
    document.getElementById('counter-menu').style.display = 'block';
    document.getElementById('counter-name').value = '';
    document.getElementById('counter-goal').value = '';
    document.getElementById('enable-goal').checked = false;
    document.getElementById('enable-keyboard').checked = false;
}

function closeCounterMenu() {
    document.getElementById('counter-menu').style.display = 'none';
}

function addCounter() {
    const counterName = document.getElementById('counter-name').value;
    const counterGoal = document.getElementById('counter-goal').value;
    const enableGoal = document.getElementById('enable-goal').checked;
    const enableKeyboard = document.getElementById('enable-keyboard').checked;

    if (counterName) {
        const counter = {
            name: counterName,
            count: 0,
            goal: enableGoal ? parseInt(counterGoal) : null,
            element: null,
            shortcutKey: null
        };

        if (enableKeyboard) {
            setupKeyboardShortcut(counter);
        } else {
...
function setupKeyboardShortcut(counter) {
    const button = document.createElement('button');
    button.className = 'btn counter-btn';
    button.textContent = 'يرجى اختيار الزر...';
    counterList.appendChild(button);
    counter.element = button;

    function handleKeyPress(event) {
        event.preventDefault();
        counter.shortcutKey = event.key;
        button.textContent = `${counter.name}: ${counter.count}${counter.goal ? ` / ${counter.goal}` : ''} (${event.key})`;
        document.removeEventListener('keydown', handleKeyPress);

        document.addEventListener('keydown', function(event) {
            if (event.key === counter.shortcutKey) {
...
function updateCounterList() {
    ...
    counters.forEach(counter => {
        const button = document.createElement('button');
        button.className = 'btn counter-btn';
        button.textContent = `${counter.name}: ${counter.count}`;
        if (counter.goal) {
            button.textContent += ` / ${counter.goal}`;
        }
        if (counter.shortcutKey) {
            button.textContent += ` (${counter.shortcutKey})`;
        }
...
function incrementCounter(counter) {
    counter.count++;
    counter.element.textContent = `${counter.name}: ${counter.count}`;
    if (counter.goal && counter.count >= counter.goal) {
        alert(`بارك الله فيك , وصلت الى النهاية: ${counter.goal}`);
    }
}

// تفعيل/إيقاف الهدف
document.getElementById('enable-goal').addEventListener('change', function() {
    const goalInput = document.getElementById('counter-goal');
    goalInput.disabled = !this.checked;
});

// تفعيل/إيقاف الكيبورد
document.getElementById('enable-keyboard').addEventListener('change', function() {
    const keyboardInput = document.getElementById('keyboard-shortcut');
    keyboardInput.disabled = !this.checked;
});
...
function showCompletionMessage(type) {
    ...
    completionMessage.innerHTML = `
        <h2>تم الانتهاء من الأذكار</h2>
        <p>حفظك الله ورعاك ووفقك</p>
        <button class="btn home-return-btn" onclick="goBack()">العودة للصفحة الرئيسية</button>
    `;
